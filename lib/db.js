...
export default {
  ...
  async getClaimWithCampaignInfo(id) {
    ...
    const { data: campaignData, error: campaignError } = await supabase
      .from('campaigns')
      .select('token_address, token_symbol, token_decimals, title, status')
  ...
  async createCampaign(campaignData) {
    ...
    const validation = await this.validateERC20Contract(campaignData.tokenAddress);
    
    if (!validation.isValid) {
      console.error('[DB] ERC-20 validation failed:', validation.error);
      throw new Error(`Invalid ERC-20 token: ${validation.error}`);
    }
  ...
  async validateERC20Contract(tokenAddress) {
    ...
    try {
      ...
      // Try to call essential ERC-20 functions
      ...
      // Check if essential functions exist and return valid data
      if (!name || !symbol || decimals === null || !totalSupply) {
        return {
          isValid: false,
          error: 'Contract does not implement required ERC-20 functions'
        };
      }
      
      // Additional validation: decimals should be reasonable (0-18)
      if (decimals > 18) {
        return {
          isValid: false,
          error: 'Token decimals exceed maximum (18)'
        };
      }
