...
export default {
  ...
  async findNativeFundingTransaction(provider, fromWallet, toWallet, requiredAmount, fromDate, chainId) {
    ...
    try {
      ...
      // For native token transfers, we need to scan block by block
      // This is expensive, so we limit the range
      const maxBlocksToScan = Math.min(blockRange, 10000); // Limit to 10k blocks for performance
      const searchFromBlock = Math.max(currentBlock - maxBlocksToScan, fromBlock);
      
      console.log('[DB] Limited search: scanning', maxBlocksToScan, 'blocks from', searchFromBlock);
      
      // Search recent blocks for native transfers
      for (let blockNumber = searchFromBlock; blockNumber <= currentBlock; blockNumber += 1000) {
        const endBlock = Math.min(blockNumber + 999, currentBlock);
        
        try {
          const block = await provider.getBlock(endBlock, true); // Get block with transactions
          ...
          for (const tx of block.transactions) {
            ...
            if (tx.from && tx.to && 
              ...
              // Check if amount matches (with small tolerance for gas differences)
          ...
          console.log('[DB] Error scanning block', blockNumber, ':', blockError.message);
          continue;
        }
      }
      ...
      return {
        found: false,
        details: `No native token transfer of ${requiredAmount} found from ${fromWallet} to ${toWallet} in the last ${maxBlocksToScan} blocks. Please ensure you sent the exact amount from your connected wallet.`
      };
