...
class Database {
  ...
  async validateERC20Contract(tokenAddress, rpcUrl = null) {
    ...
    try {
      ...
      // Create provider without network config to avoid chainId issues
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          amount: parseFloat(transaction.value || transaction.amount),
  ...
  async findNativeFundingTransaction(provider, fromWallet, toWallet, requiredAmount, fromDate, chainId) {
    ...
    try {
      ...
      // If the destination wallet has at least the required amount, funding is sufficient
      if (currentToBalanceFloat >= requiredAmount) {
        console.log('[DB] Balance-based verification: destination wallet has sufficient balance (', currentToBalanceFloat, '>=', requiredAmount, ')');
        
        // Get current block number for the record
        const currentBlockNumber = await provider.getBlockNumber();
        
        return {
          found: true,
          transaction: {
            hash: `balance-verified-${Date.now()}`, // Unique placeholder hash
            from: fromWallet,
            to: toWallet,
            amount: currentToBalanceFloat,
            blockNumber: currentBlockNumber, // Use current block number instead of 'unknown'
            value: currentToBalanceFloat, // Add value field for recordCampaignDeposit
            verificationMethod: 'balance-based'
          },
          amount: currentToBalanceFloat,
          note: `Funding verified by destination balance (${currentToBalanceFloat} MON >= ${requiredAmount} MON required). Direct transaction search may have missed older blocks.`
        };
      }
